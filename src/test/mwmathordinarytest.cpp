...
int main(int argc, const char* argv[]) {
    ...
    LOG_INFO(_logger, "big_matrix.get_matrix_info() = " <<
             big_matrix.get_matrix_info());
    LOG_INFO(_logger, "big_matrix.get_row_num() = " << big_matrix.get_row_num());
    LOG_INFO(_logger, "big_matrix.get_column_num() = " <<
             big_matrix.get_column_num());
    ...
    LOG_INFO(_logger, "big_matrix_2.get_matrix_info() = " <<
             big_matrix_2.get_matrix_info());
    LOG_INFO(_logger, "big_matrix_2.get_row_num() = " <<
             big_matrix_2.get_row_num());
    LOG_INFO(_logger, "big_matrix_2.get_column_num() = " <<
             big_matrix_2.get_column_num());
    ...
    LOG_INFO(_logger, "LinearFunction::get_instance()->get_value(" << v1 << ") = "
             << LinearFunction::get_instance()->get_value(v1));
    LOG_INFO(_logger, "LinearFunction::get_instance()->get_value_matrix(" << m4 <<
             ") = " << LinearFunction::get_instance()->get_value_matrix(m4));
    LOG_INFO(_logger, "LinearFunction::get_instance()->get_value_vector_matrix(" <<
             vector_matrix << ") = " <<
             LinearFunction::get_instance()->get_value_vector_matrix(vector_matrix));
    LOG_INFO(_logger, "LinearFunction::get_instance()->get_derivative(" << v1 <<
             ") = " << LinearFunction::get_instance()->get_derivative(v1));
    LOG_INFO(_logger, "LinearFunction::get_instance()->get_derivative_matrix(" << m4
             << ") = " << LinearFunction::get_instance()->get_derivative_matrix(m4));
    LOG_INFO(_logger,
             "LinearFunction::get_instance()->get_derivative_vector_matrix(" << vector_matrix
             << ") = " << LinearFunction::get_instance()->get_derivative_vector_matrix(
                 vector_matrix));

    LOG_INFO(_logger, "LogsigFunction::get_instance()->get_value(" << v1 << ") = "
             << LogsigFunction::get_instance()->get_value(v1));
    LOG_INFO(_logger, "LogsigFunction::get_instance()->get_value_matrix(" << m4 <<
             ") = " << LogsigFunction::get_instance()->get_value_matrix(m4));
    LOG_INFO(_logger, "LogsigFunction::get_instance()->get_value_vector_matrix(" <<
             vector_matrix << ") = " <<
             LogsigFunction::get_instance()->get_value_vector_matrix(vector_matrix));
    LOG_INFO(_logger, "LogsigFunction::get_instance()->get_derivative(" << v1 <<
             ") = " << LogsigFunction::get_instance()->get_derivative(v1));
    LOG_INFO(_logger, "LogsigFunction::get_instance()->get_derivative_matrix(" << m4
             << ") = " << LogsigFunction::get_instance()->get_derivative_matrix(m4));
    LOG_INFO(_logger,
             "LogsigFunction::get_instance()->get_derivative_vector_matrix(" << vector_matrix
             << ") = " << LogsigFunction::get_instance()->get_derivative_vector_matrix(
                 vector_matrix));

    RandomFunction::get_instance()->initialize();
    LOG_INFO(_logger, "RandomFunction::get_instance()->get_value(" << v1 << ") = "
             << RandomFunction::get_instance()->get_value(v1));
    LOG_INFO(_logger, "RandomFunction::get_instance()->get_value_matrix(" << m4 <<
             ") = " << RandomFunction::get_instance()->get_value_matrix(m4));
    LOG_INFO(_logger, "RandomFunction::get_instance()->get_value_vector_matrix(" <<
             vector_matrix << ") = " <<
             RandomFunction::get_instance()->get_value_vector_matrix(vector_matrix));
    ...
    nf_test[4] = 2;
    nf_test[0] = 130;
    LOG_INFO(_logger, "NormalizeFunction::get_instance()->get_value(" << nf_test <<
             ") = " << NormalizeFunction::get_instance()->get_value(nf_test));
    ...
    for (size_t i = 1, j; i < 100; ++ i) {
        ...
        a_matrix = FixedAreaSinByCountFunction::get_instance()->get_value_matrix(
                       p_matrix);
        square = 0;
        for (j = 0; j < i; ++ j) {
            square += a_matrix._data[j][0];
        }
        LOG_INFO(_logger, "FixedAreaSinByCountFunction: i = " << i << ", a_matrix " <<
                 a_matrix << ", square = " << square);
    }
    ...
    for (size_t i = 1, j; i < 100; ++ i) {
        ...
        a_matrix = FixedAreaSinUpByCountFunction::get_instance()->get_value_matrix(
                       p_matrix);
        square = 0;
        for (j = 0; j < i; ++ j) {
            square += a_matrix._data[j][0];
        }
        LOG_INFO(_logger, "FixedAreaSinUpByCountFunction: i = " << setprecision(
                     20) << i << ", a_matrix " << a_matrix << ", square = " << square);
    }
    ...
    LOG_INFO(_logger, "(vector_matrix = " << vector_matrix <<
             ") - (vector_matrix_2 = " << vector_matrix_2 << ") = (vector_matrix_3 = " <<
             vector_matrix_3 << ")");

    assign_times(vector_matrix, 2, vector_matrix_3);
    LOG_INFO(_logger, "(vector_matrix = " << vector_matrix <<
             ") * 2 = (vector_matrix_3 = " << vector_matrix_3 << ")");

    assign_times(vector_matrix, m11, vector_matrix_3);
    LOG_INFO(_logger, "(vector_matrix = " << vector_matrix << ") * (m11 = " << m11
             << ") = (vector_matrix_3 = " << vector_matrix_3 << ")");
    ...
    LOG_INFO(_logger, "(m12 = " << m12 << ") * (m11.t() = " << m11.t() <<
             ") = (m13 = " << m13 << ")");

    assign_diagonaled_times_transformed(vector_matrix, m13, vector_matrix_3);
    LOG_INFO(_logger, "(vector_matrix = " << vector_matrix << ") * (m13 = " <<
             m13.t() << ") assign_diagonaled_times_transformed= (vector_matrix_3 = " <<
             vector_matrix_3 << ")");
