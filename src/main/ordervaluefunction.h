...
class OrderValueFunction : public MWUnderivableMathFunction,
    public MWSingleton<OrderValueFunction> {
  private:
    std::map<MWData, MWData> _value_map;
    MWData _min_value;
    MWData _max_value;

  public:
    void Initialize(const MWVector<MWVector<MWMatrix<MWData> > > &data_vector_vector, 
                    MWData min_value, 
                    MWData max_value);

    virtual MWVector<MWData> GetValueStructure(const MWVector<MWData> &input) const;
